...
namespace Evolution.Forms
{
    public partial class OwnerCircleCommision : Form
    {
        ...
        {       
            ...
            {
                if (CBTranType.Text == "" || CBTranType.SelectedValue == null) { MessageBox.Show("Invalid NC Type", "Owner", MessageBoxButtons.OK, MessageBoxIcon.Warning); CBTranType.Focus(); return; }
            }
            /*--------------------------------------------------------------------------------------------*/
            try
            {
                ...
                DVSave = SQLCMD.SQLdata("LS_MemberServiceFeePaymentDetail_ML 0,0,0,'" + StartDate.Value + "','" + EndDate.Value + "'," + DVSearch.Table.Rows[0]["AgreementID"].ToString() + "," +
                  ...
                  ((ckbTransfer.CheckState == CheckState.Checked)? 1 : 2) +","+((CBTranType.Text=="" || CBTranType.SelectedValue ==null)? "Null" : CBTranType.SelectedValue)+"").DefaultView;
                /*-----------------------------------------*/
                FillGrid();
                GRDHistory.DataSource = DVHistiry;
                SumNight();
        ...
        private void OwnerCircleCommision_Load(object sender, EventArgs e)
        {
            ...
            //CBTranType.DataSource = SQLCMD.SQLdata("LS_OwnerCircleNcType_SPL 1").DefaultView;
            CBTranType.DataSource = SQLCMD.SQLdata("LS_OwnerCircleNcType_SPL_V3 1").DefaultView;

            CBTranType.DisplayMember = "Name";
            CBTranType.ValueMember = "ID";
            CBTranType.Text = "";
        ...
        {
            if (DVSearch.Count < 1) { return; }
            DVSave = SQLCMD.SQLdata("LS_MemberServiceFeeSearch_L 2,null,null,null,null,"+DVSearch.Table.Rows[0]["Agreementid"].ToString()+",null,1,9999999999,'01-01-1990','01-01-3000'").DefaultView;
        ...
        private void ckbTransfer_CheckStateChanged(object sender, EventArgs e)
        {
            ...
            CBTranType.Visible = ((ckbTransfer.CheckState == CheckState.Checked) ? false : true);
        }
        ...
        private void ckbReceipt_CheckStateChanged(object sender, EventArgs e)
        {
            ...
            PnGenerateCredit.Visible = ((ckbReceipt.CheckState == CheckState.Checked) ? true : false);
            //CBTranType.Visible = ((ckbReceipt.CheckState == CheckState.Checked) ? true : false);
        }
